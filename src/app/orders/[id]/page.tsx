...
export default function OrderDetailPage({
  ...
  const getStatusLabel = (status: string) => {
    switch (status?.toLowerCase()) {
      ...
      case "reached_stockist": return "Reçue";
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} alignItems={{ xs: 'flex-start', sm: 'center' }} justifyContent="space-between">
          ...
          <Stack direction="row" spacing={1.5} flexWrap="wrap">
            ...
            {order.status === 'shipped_to_stockist' && (
              ...
              >
                {updating ? <CircularProgress size={20} /> : 'Réception confirmée'}
              </Button>
            )}
            {order.status === 'reached_stockist' && (
              <Button
                ...
                sx={{ borderRadius: '12px', px: 3 }}
              >
                {updating ? <CircularProgress size={20} /> : 'Mettre en livraison'}
              </Button>
            )}
            {order.status === 'out_for_delivery' && (
              <Button
                ...
                sx={{ borderRadius: '12px', px: 3 }}
              >
                {updating ? <CircularProgress size={20} /> : 'Marquer comme livrée'}
              </Button>
            )}
          </Stack>
        </Stack>
