...
export const { handlers, auth, signIn, signOut } = NextAuth({
            ...
            authorize: async (credentials) => {
                try {
                    ...
                    if (response.data.status === 200 && response.data.user.token) {
                        ...
                        // Restriction: Only Stockist users allowed in this panel
                        if (response.data.user.role !== 'stockist') {
                            console.log("AUTH_DEBUG: Access denied - role mismatch (expected stockist)");
                            return null;
                        }
    ...
    cookies: {
        sessionToken: {
            name: `next-auth.session-token.stockist`,
            options: {
                httpOnly: true,
                sameSite: "lax",
                path: "/",
